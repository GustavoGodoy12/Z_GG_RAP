CLASS zcl_gg_cargas DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_gg_cargas IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    TYPES: tt_product  TYPE TABLE OF ztgg_product WITH DEFAULT KEY,
           tt_vendor   TYPE TABLE OF ztgg_vendor WITH DEFAULT KEY,
           tt_company  TYPE TABLE OF ztgg_company WITH DEFAULT KEY,
           tt_customer TYPE TABLE OF ztgg_customer WITH DEFAULT KEY.
    ...
    DELETE FROM ztgg_product.
    DELETE FROM ztgg_vendor.
    DELETE FROM ztgg_company.
    DELETE FROM ztgg_customer.

    MODIFY ztgg_product FROM TABLE @lt_product.
    MODIFY ztgg_vendor FROM TABLE @lt_vendor.
    MODIFY ztgg_company FROM TABLE @lt_company.
    MODIFY ztgg_customer FROM TABLE @lt_customer.
